...
async function findAll({ includeInactive=false, carreraPeriodoId=null }={}){
  ...
  const [rows]=await pool.query(`SELECT * FROM rubrica r ${ws} ORDER BY r.created_at DESC`, params);
  return rows;
}
...
async function create(d){
  const [res]=await pool.query(
    `INSERT INTO rubrica (id_carrera_periodo, nombre_rubrica, descripcion_rubrica, estado)
     VALUES (?,?,?,1)`,
    [d.id_carrera_periodo, d.nombre_rubrica, d.descripcion_rubrica ?? null]
  );
...
async function update(id, d){
  await pool.query(
    `UPDATE rubrica SET id_carrera_periodo=?, nombre_rubrica=?, descripcion_rubrica=?
     WHERE id_rubrica=?`,
    [d.id_carrera_periodo, d.nombre_rubrica, d.descripcion_rubrica ?? null, id]
  );
  return findById(id);
}
