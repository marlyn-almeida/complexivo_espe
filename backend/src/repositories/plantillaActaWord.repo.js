...
async function activar(id) {
  ...
  try {
    ...
    await conn.query(
      `UPDATE plantilla_acta_word SET estado_activa = 0 WHERE estado = 1`
    );

    const [res] = await conn.query(
      `UPDATE plantilla_acta_word
       SET estado_activa = 1
    ...
    if (res.affectedRows === 0)
      throw new Error("Plantilla no encontrada o eliminada.");
