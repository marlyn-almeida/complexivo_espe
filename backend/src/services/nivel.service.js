const repo = require("../repositories/nivel.repo");
async function list(q){ return repo.findAll({ includeInactive: q.includeInactive==="true" }); }
async function get(id){ const r=await repo.findById(id); if(!r){const e=new Error("Nivel no encontrado"); e.status=404; throw e;} return r; }
