...
// ✅ NUEVO DOCENTE: mis items (por id_tribunal_estudiante)
async function misItemsTribunal(req, res, next) {
  try {
    const cp = Number(req.ctx.id_carrera_periodo);
    const id_te = Number(req.params.id_tribunal_estudiante);
    const data = await svc.misItemsTribunal(cp, id_te, req.user);
    res.json(data);
  } catch (e) {
    next(e);
  }
