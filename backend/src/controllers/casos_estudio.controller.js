...
// ✅ ✅ ✅ NUEVO: borrar real (admin)
async function remove(req, res, next) {
  try {
    const cp = Number(req.ctx.id_carrera_periodo);
    const id_caso_estudio = Number(req.params.id_caso_estudio);

    await svc.remove(cp, id_caso_estudio);

    // (opcional) aquí podrías borrar el archivo físico, pero solo si quieres.
    // OJO: si lo borras, y el caso ya no existe, no hay problema.
    res.json({ ok: true });
  } catch (e) {
    next(e);
  }
